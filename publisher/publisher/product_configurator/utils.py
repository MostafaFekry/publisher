...
def get_products_for_website(field_filters=None, attribute_filters=None, search=None,search_category='General'):
	...
	if search:
		return get_items(search=search,search_category=search_category)

	return get_items()

def get_products_for_website_count(field_filters=None, attribute_filters=None, search=None, search_category='General'):

	if attribute_filters:
		item_codes = get_item_codes_by_attributes(attribute_filters)
		items_by_attributes = get_items_count([['name', 'in', item_codes]])

	if field_filters:
		items_by_fields = get_items_by_fields_count(field_filters)
	...
	if search:
		return (get_items_count(search=search,search_category=search_category))

	return (get_items_count())
...
def get_products_html_for_website(field_filters=None, attribute_filters=None):
	...
	if not items:
		html = frappe.render_template('publisher/www/all-books/not_found.html', {})
...
def get_items_by_fields_count(field_filters):
	meta = frappe.get_meta('Item')
	filters = []
	for fieldname, values in field_filters.items():
		if not values: continue
	...
	return get_items_count(filters)


def get_items(filters=None, search=None,search_category='General'):

	sort_by = frappe.form_dict.sort or ''
	products_settings = get_product_settings()
	page_length = products_settings.products_per_page or 10
	start = 0
	if frappe.form_dict.pg:
		start = (cint(frappe.form_dict.pg)*page_length)-page_length


	#set order_by value
	if sort_by == 'date':
		order_by = "`publication_date` DESC"
	elif sort_by == 'namease':
		order_by = "`item_name`"
...
def get_items_count(filters=None, search=None,search_category='General'):
	products_settings = get_product_settings()
	...
	results = frappe.db.sql('''
		SELECT
			count(`tabItem`.`name`) as itemcount
		FROM
			`tabItem`
		{left_join}
		WHERE
...
def get_html_for_items(items):
	...
	all_books_column_value = cint(12 / cint(doc.no_of_columns_all_books or 4))
	show_price = frappe.db.get_single_value('Shopping Cart Settings', 'show_price')
	html = []
	for item in items:
		html.append(frappe.render_template('publisher/www/all-books/item_row.html', {
			'item': item,'all_books_column_value':all_books_column_value,'display_all_books_as':doc.display_all_books_as or 'Grid','show_price':show_price
		}))
	return html
