...
def get_context(context):
	...
	context.title = context.doc.title or "Contact us"
	context.column_value = cint(12 / cint(context.doc.no_of_columns or 3))
	context.number_of_columns_branches = context.doc.number_of_columns_item or 4

	if context.doc.query_options:
		query_options = [opt.strip() for opt in context.doc.query_options.replace(",", "\n").split("\n") if opt]
	else:
