...
class Blog(WebsiteGenerator):
	...
	def make_route(self):
		if not self.route:
			return frappe.db.get_value('Website Blog Category', self.blog_category,
	...
	def validate(self):
		...
		# update posts
		frappe.db.sql("""UPDATE `tabWebsite Blogger` SET `posts`=(SELECT COUNT(*) FROM `tabBlog`
			WHERE IFNULL(`blogger`,'')=`tabWebsite Blogger`.`name`)
...
def get_list_context(context):
	...
	if category:
		...
		context.sub_title = _("Posts filed under {0}").format(category_title)
		context.title = category_title

	elif frappe.local.form_dict.blogger:
		blogger = frappe.db.get_value("Website Blogger", {"name": frappe.local.form_dict.blogger}, "full_name")
		context.sub_title = _("Posts by {0}").format(blogger)
		context.title = blogger

	elif frappe.local.form_dict.txt:
		context.sub_title = _('Filtered by "{0}"').format(sanitize_html(frappe.local.form_dict.txt))

	if context.sub_title:
		context.parents = [{"name": _("Home"), "route": "/"},
								{"name": "Blog", "route": "/blog"}]
	else:
		context.parents = [{"name": _("Home"), "route": "/"}]

	context.update(frappe.get_doc("Blog Settings", "Blog Settings").as_dict(no_default_fields=True))

def get_children():
	return frappe.db.sql("""select route as name,
		title from `tabWebsite Blog Category`
		where published = 1
		and exists (select name from `tabBlog`
			where `tabBlog`.blog_category=`tabWebsite Blog Category`.name and published=1)
		order by title asc""", as_dict=1)

def clear_blog_cache():
	for blog in frappe.db.sql_list("""select route from
		`tabBlog` where ifnull(published,0)=1"""):
		clear_cache(blog)

	clear_cache("writers")

def get_blog_category(route):
	return frappe.db.get_value("Website Blog Category", {"name": route}, "title") or route
...
def get_blog_list(doctype, txt=None, filters=None, limit_start=0, limit_page_length=20, order_by=None):
	...
	query = """\
		select
			t1.title, t1.name, t1.blog_category, t1.route, t1.published_on, t1.main_image,
				...
				(select count(name) from `tabComment`
					where
						...
						and reference_doctype='Blog'
						and reference_name=t1.name) as comments
		from `tabBlog` t1, `tabWebsite Blogger` t2
	...
	for post in posts:
		...
		if post.main_image:
			post.cover_image = post.main_image
		else:
		...
		post.category = frappe.db.get_value('Website Blog Category', post.blog_category,
